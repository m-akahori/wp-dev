	...
	return array(
		...
		'category_list' => array(
			...
			'default' => array(
				...
				'cb_desc_font_size_mobile' => 12,
				'cb_categories' => array(),
				'cb_categories_txtcolor' => '#000000',
				'cb_categories_bgcolor' => '#F7F7F7',
				'cb_categories_hvcolor' => '#EEEEEE'
			)
		),
		'blog_list' => array(
			'name' => 'blog_list',
...
<?php
		...
		$taxonomy_objects = get_object_taxonomies( $dp_options['introduce_slug'], 'objects' );
		if ( $taxonomy_objects ) :
...
?>
		...
		<div class="cb_content">
			...
			<h4 class="theme_option_headline2"><?php _e( 'Categories to display', 'tcd-w' ); ?></h4>
			<p><?php _e('The category list is displayed from the left from the regular article category and the introduction page category in that order, so please check the category to display.','tcd-w'); ?></p>
			<p><?php _e('The name of category (name of Category 1 ~ 3) can be changed arbitrarily from category name by TCD theme option → blog (introduction page) → blog (introduction page) category setting.','tcd-w'); ?></p>
			<p><?php _e('If you want to display category images, please register a category image on the setting screen of each category.','tcd-w'); ?></p>
...
?>
			...
			<h4 class="theme_option_headline2"><?php _e( 'Color of category list items', 'tcd-w' ); ?></h4>
			<ul class="headline_option">
				<li>
					<span class="label"><?php _e('Text color', 'tcd-w'); ?></span><input type="text" name="dp_options[contents_builder][<?php echo $cb_index; ?>][cb_categories_txtcolor]" value="<?php echo esc_attr( $value['cb_categories_txtcolor'] ); ?>" class="c-color-picker" data-default-color="<?php echo esc_attr($cb_contents[$cb_content_select]['default']['cb_categories_txtcolor']); ?>">
				</li>
				<li>
					<span class="label"><?php _e('Background color', 'tcd-w'); ?></span><input type="text" name="dp_options[contents_builder][<?php echo $cb_index; ?>][cb_categories_bgcolor]" value="<?php echo esc_attr( $value['cb_categories_bgcolor'] ); ?>" class="c-color-picker" data-default-color="<?php echo esc_attr($cb_contents[$cb_content_select]['default']['cb_categories_bgcolor']); ?>">
				</li>
				<li>
					<span class="label"><?php _e('Background hover color', 'tcd-w'); ?></span><input type="text" name="dp_options[contents_builder][<?php echo $cb_index; ?>][cb_categories_hvcolor]" value="<?php echo esc_attr( $value['cb_categories_hvcolor'] ); ?>" class="c-color-picker" data-default-color="<?php echo esc_attr($cb_contents[$cb_content_select]['default']['cb_categories_hvcolor']); ?>">
				</li>
			</ul>
	...
	if ( ! empty( $input['contents_builder'] ) ) {
		...
		foreach( $input['contents_builder'] as $key => $value ) {
			...
			// 最新ブログ記事一覧
				...
				$value['cb_post_term'] = intval( $value['cb_post_term'] );
				if ( ! empty( $value['cb_list_type'] ) && ! isset( $cb_contents[$value['cb_content_select']]['cb_list_type_options'][$value['cb_list_type']] ) ) {
					$value['cb_list_type'] = null;
				}
				if ( empty( $value['cb_list_type'] ) && isset( $cb_contents[$value['cb_content_select']]['default']['cb_list_type'] ) ) {
